...
void updateMouse() {
  ...
  if(lastmy <= height*2/3) {
    ...
    if(lastmx < width/2 && mouseState == 2 && sqrt(sq(mouseX-lastmx)+sq(mouseY-lastmy)) >= 10) {
      ...
      tmp = returnRenderSegment(new Segment((ccurx-1)*CUBE_SIZE, (ccury-1)*CUBE_SIZE, (ccurz-1)*CUBE_SIZE,
                                            (ccurx-1)*CUBE_SIZE+100, (ccury-1)*CUBE_SIZE, (ccurz-1)*CUBE_SIZE, 0), control[1], control[0], 0, 0);
      ...
      tmp = returnRenderSegment(new Segment((ccurx-1)*CUBE_SIZE, (ccury-1)*CUBE_SIZE, (ccurz-1)*CUBE_SIZE,
                                            (ccurx-1)*CUBE_SIZE, (ccury-1)*CUBE_SIZE+100, (ccurz-1)*CUBE_SIZE, 0), control[1], control[0], 0, 0);
      ...
      tmp = returnRenderSegment(new Segment(0, 0, (ccurz-1)*CUBE_SIZE, 0, 0, (ccurz-1)*CUBE_SIZE+100, 0), control[1], control[0], 0, 0);
      float zpd = abs(radDiff(ptsToRad(tmp[0], tmp[1], tmp[2], tmp[3]), ptsToRad(lastmx, lastmy, mouseX, mouseY)));
      if(zpd<curmin) {curmin=zpd; curmind=4;}
      float znd = PI-zpd;
      if(znd<curmin) {curmin=znd; curmind=5;}
      ...
      switch(curmind) {
        ...
        case 4: keypressHandler('q'); break;
        case 5: keypressHandler('e'); break;
      }
    }
  }
